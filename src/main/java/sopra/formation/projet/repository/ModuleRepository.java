...
public interface ModuleRepository extends JpaRepository<Module,Integer>{
	...
	@Query("select distinct m from Module m left join fetch m.modulesStagiaires ms left join fetch ms.key.stagiaire")
	List<Module> findModuleWithStagiare();
	
	@Query("select m from Module m left join fetch m.modulesStagiaires ms left join fetch ms.key.stagiaire where m.idModule=:num")
